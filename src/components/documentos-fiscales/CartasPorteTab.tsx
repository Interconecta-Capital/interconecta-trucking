...
import { FileText, Download, Eye, Truck, Calendar } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <Badge variant="secondary">{cartasPorte?.length || 0} cartas porte</Badge>
      </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <Badge variant={
                        cp.status === 'timbrada' ? 'default' :
                        cp.status === 'cancelada' ? 'destructive' :
                        'secondary'
                      }>
                        {cp.status}
                      </Badge>
                    ...
                    {cp.uuid_fiscal && (
                      <p className="text-sm text-muted-foreground font-mono mb-2" title={cp.uuid_fiscal}>
                        UUID: {cp.uuid_fiscal}
                      </p>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-mono">{cp.rfc_emisor}</p>
                  </div>
                  <div>
                    <p className="font-medium text-muted-foreground">RFC Receptor</p>
                    <p className="font-mono">{cp.rfc_receptor}</p>
                  </div>
                  <div>
                    <p className="font-medium text-muted-foreground">Fecha</p>
                    <div className="flex items-center gap-2">
                      <Calendar className="h-4 w-4" />
                      <span>{new Date(cp.created_at).toLocaleDateString('es-MX')}</span>
                    </div>
                  </div>
                </div>
                ...
                {cp.distancia_total && (
                  <div className="mt-3">
                    <p className="text-sm">
                      <span className="font-medium">Distancia total:</span> {cp.distancia_total} km
                    </p>
                  </div>
                )}
                ...
                {cp.viaje && (
                  <div className="mt-4 pt-4 border-t">
                    ...
                    <p className="text-sm font-medium">{cp.viaje.origen} → {cp.viaje.destino}</p>
                  </div>
                )}
