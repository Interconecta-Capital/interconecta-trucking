...
export const FormularioCodigoPostalMexicano: React.FC<FormularioCodigoPostalMexicanoProps> = ({
  onDomicilioChange,
  onDomicilioCompleto,
  valorInicial = {},
  mostrarPreview = true,
  ...
  const actualizarCampo = useCallback((campo: keyof DatosDomicilio, valor: string) => {
    setFormData(prev => {
      ...
      // Si el domicilio está completo, notificar
      if (nuevosDatos.codigoPostal && nuevosDatos.estado && nuevosDatos.municipio && 
        ...
        console.log('[DOMICILIO_COMPLETO]', nuevosDatos);
  ...
  const handleCodigoPostalChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (valor.length === 5) {
      ...
      // Limpiar datos si el CP es incompleto
...
export default FormularioCodigoPostalMexicano;
