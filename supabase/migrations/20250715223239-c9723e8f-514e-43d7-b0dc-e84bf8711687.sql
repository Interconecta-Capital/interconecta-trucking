-- Sincronización FINAL de estados entre viajes y recursos (con todos los estados correctos)
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.estado != NEW.estado) THEN
    ...
    -- Actualizar estado del conductor (con estados válidos)
    ...
    -- Actualizar estado del vehículo (con estados válidos)
...
-- Sincronizar estados de conductores (con estados válidos)
UPDATE public.conductores 
SET 
  estado = CASE v.estado
    WHEN 'programado' THEN 'disponible'    -- Mantener disponible para programado
