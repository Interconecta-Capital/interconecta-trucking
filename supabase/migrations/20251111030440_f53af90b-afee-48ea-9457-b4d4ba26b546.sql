-- Migración para corregir funciones de eliminación de datos
-- Problema: Las funciones intentan insertar en columna 'severity' que no existe en security_audit_log

-- 1. Recrear función anonimizar_usuario (sin columna severity)
...
BEGIN
  ...
  -- Registrar auditoría (SIN columna severity)
...
-- 2. Recrear función eliminar_datos_usuario (sin columna severity)
...
BEGIN
  ...
  -- Registrar en security_audit_log (SIN columna severity)
  INSERT INTO public.security_audit_log (
    user_id,
    event_type,
    event_data
