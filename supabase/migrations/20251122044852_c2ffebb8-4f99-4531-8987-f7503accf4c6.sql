-- Actualizar función RPC para cargar mercancías del viaje correctamente
...
BEGIN
  SELECT json_build_object(
    ...
    -- ⚡ UNIFICADO: Cargar mercancías directamente del viaje (no del carta porte)
    'mercancias', COALESCE(
      ...
      FROM mercancias m
      WHERE m.viaje_id = v.id), -- ⚡ Ahora busca por viaje_id, no carta_porte_id
      '[]'::json
    )
  ) INTO v_result
  FROM viajes v
...
COMMENT ON FUNCTION public.get_viaje_con_relaciones(uuid) IS 'Obtiene viaje completo - UNIFICADO: mercancías por viaje_id, no carta_porte_id';